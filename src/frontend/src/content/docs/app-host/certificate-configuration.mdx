...
- **Windows** — Aspire passes the developer certificate thumbprint to DCP via `--tls-cert-thumbprint`.
- **macOS and Linux** — Aspire exports the certificate and private key to cached files (stored under your user profile with user-only permissions) and passes `--tls-cert-file`, `--tls-key-file`, and `--tls-cert-thumbprint` to DCP.
