...
The most common scenario is to call commands from within other custom command implementations. The `ExecuteCommandContext` provides access to the `IServiceProvider` (C#) or `serviceProvider()` helper (TypeScript), which you can use to retrieve the `ResourceCommandService`.
...
const api = await builder.addProject("api", { projectPath: "../Api/Api.csproj" });
