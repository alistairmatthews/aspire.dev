...
**Solution**: Make sure you're using <ApiReference name="WaitFor()" /> in your AppHost:
...
**Solution**: Verify you're using <ApiReference name="WithReference()" /> to connect resources.
...
- The consuming service has <ApiReference name=".WithReference(api)" /> in the AppHost
