...
2. Ask your AI coding agent to run the `aspireify` skill. The agent will:
   ...
   - Wire resources into the AppHost with <ApiReference name="WithReference" />, <ApiReference name="WaitFor" />, endpoints, and volumes
...
If a workload does not have a dedicated hosting API yet, model it as an executable resource with <ApiReference name="AddExecutable" /> so it can still participate in the same application model.
