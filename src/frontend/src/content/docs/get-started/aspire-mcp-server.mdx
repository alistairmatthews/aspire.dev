...
By default, all resources, console logs, and telemetry are accessible through the MCP server. You can exclude specific resources and their associated telemetry by annotating them with <ApiReference name="ExcludeFromMcp()" />:
...
- **Granular access control** — use <ApiReference name="ExcludeFromMcp()" /> to restrict sensitive resources
