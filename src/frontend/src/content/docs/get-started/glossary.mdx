...
<ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WithReference" /> is how you connect resources together. When you reference another resource, Aspire:
...
<Aside type="tip">
  Without waiting for the database to become ready, your API might start and immediately crash. Waiting for readiness prevents race conditions during startup.
</Aside>
...
<Aside type="tip">
  Wait only for the running state when you need looser coupling—your service can handle initial connection failures via retry policies. Wait for health checks to pass when your service requires a healthy dependency.
</Aside>
...
- Works automatically when you connect resources with a reference
...
1. **AppHost level**: Determines when dependencies are ready so resources waiting for them can start
