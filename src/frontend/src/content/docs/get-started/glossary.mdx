...
<ApiReference name="WithReference()" /> is how you connect resources together. When you call <ApiReference name=".WithReference(otherResource)" />, Aspire:
...
<ApiReference name="WaitFor()" /> tells Aspire to delay starting a resource until its dependency is ready:
...
<Aside type="tip">
  Without <ApiReference name="WaitFor()" />, your API might start and immediately crash because the database isn't ready yet. This prevents race conditions during startup.
</Aside>

### WaitForCompletion

<ApiReference name="WaitForCompletion()" /> waits for a resource to finish and exit (not just start). Useful for setup scripts:
...
<ApiReference name="WaitForStart()" /> waits only for a resource to reach the running state, without waiting for health checks to pass:
...
<Aside type="tip">
  Use <ApiReference name="WaitForStart()" /> when you need looser coupling—your service can handle initial connection failures via retry policies. Use <ApiReference name="WaitFor()" /> when you need strict dependency on health.
</Aside>
...
- Works automatically when you use <ApiReference name="WithReference()" />
...
1. **AppHost level**: Determines when dependencies are ready (controls <ApiReference name="WaitFor()" /> behavior)
...
- Custom values via <ApiReference name="WithEnvironment()" />
...
Example: `Aspire.Hosting.PostgreSQL` adds the <ApiReference name="AddPostgres()" /> method.
