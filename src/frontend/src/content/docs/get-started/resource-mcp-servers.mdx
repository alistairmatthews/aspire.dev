...
When a resource is annotated with <ApiReference name="WithMcpServer()" /> in the AppHost, Aspire discovers the MCP endpoint and makes its tools available in two ways:
...
Use the <ApiReference name="WithMcpServer()" /> extension method to declare that a resource hosts an MCP server:
...
<Aside type="note">
<ApiReference name="WithMcpServer()" /> and <ApiReference name="WithPostgresMcp()" /> are experimental APIs. Suppress the `ASPIREMCP001` or `ASPIREPOSTGRES001` diagnostic to use them.
</Aside>
...
You can add any container that implements the MCP protocol as an MCP-enabled resource. Use <ApiReference name="WithMcpServer()" /> to tell Aspire where the MCP endpoint lives:
...
The <ApiReference name="WithMcpServer()" /> method accepts an optional path and endpoint name:

- <ApiReference name="WithMcpServer()" /> — uses the default HTTP endpoint at the root path
- <ApiReference name='WithMcpServer("/mcp")' /> — uses the default HTTP endpoint at `/mcp`
