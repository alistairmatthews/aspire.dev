...
<SimpleAppHostCode lang="python">
  <p>Uses <ApiReference name="AddUvicornApp()" /> with <ApiReference name="WithUv()" /> for ASGI apps like FastAPI.</p>
</SimpleAppHostCode>
</Pivot>
<Pivot id="nodejs">
<SimpleAppHostCode lang="nodejs">
  <p>Uses <ApiReference name="AddNodeApp()" /> with <ApiReference name="WithNpm()" /> for Node.js applications.</p>
</SimpleAppHostCode>
</Pivot>
<Pivot id="go">
...
<Aside type="tip">
  To also keep the container running between AppHost restarts (avoiding slow startup), add <ApiReference name="WithPersistentLifetime()" />. See [resource lifetimes](/app-host/resource-lifetimes/) for details.
</Aside>
...
- Define explicit dependencies with <ApiReference name="WithReference()" /> to make wiring obvious.
